...
function LoginPage() {
  ...
  return <main className="auth-page"><ThemeToggle className="auth-theme-toggle" /><section className="auth-brand"><img src={logo.url} alt="Dábliu Consórcios" /><span>CENTRAL DE RESULTADOS</span><h1>Performance comercial em tempo real.</h1><p>Vendas, equipes, relatórios e notificações reunidos em uma operação segura.</p><div className="auth-benefits"><span><BarChart3 /> Indicadores ao vivo</span><span><BellRing /> Alertas instantâneos</span><span><LockKeyhole /> Acesso por hierarquia</span></div></section><section className="auth-card"><img src={logo.url} alt="Dábliu Consórcios" className="auth-logo" /><span className="panel-kicker">ACESSO SEGURO</span><h2>{mode === "forgot" ? "Recuperar senha" : "Bem-vindo de volta"}</h2><p>{mode === "forgot" ? "Informe seu e-mail para receber o link." : "Entre com o acesso criado pelo seu gestor."}</p><form onSubmit={submit}><label>E-mail<input type="email" value={email} onChange={(e) => setEmail(e.target.value)} maxLength={200} autoComplete="email" required /></label>{mode !== "forgot" && <label>Senha<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} minLength={8} maxLength={72} autoComplete="current-password" required /></label>}{message && <div className="auth-message">{message}</div>}<button className="primary-btn auth-submit" disabled={busy}>{busy ? "Aguarde..." : <>{mode === "forgot" ? "Enviar link" : "Entrar"}<ArrowRight size={17} /></>}</button></form>{mode === "login" && <button className="auth-link" onClick={() => setMode("forgot")}>Esqueci minha senha</button>}{mode !== "login" && <button className="auth-link" onClick={() => { setMode("login"); setMessage(""); }}>Voltar para entrar</button>}</section></main>;
}
