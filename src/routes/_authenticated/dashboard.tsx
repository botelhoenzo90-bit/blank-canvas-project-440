...
function SaleModal({ onClose, onSave, saving }: { onClose: () => void; onSave: (s: Omit<Sale, "id" | "date" | "time">) => Promise<void>; saving: boolean }) { const [value, setValue] = useState(""); const submit = (e: React.FormEvent) => { e.preventDefault(); void onSave({ seller: "", supervisor: "", representative: "", master: "", team: "", value: Number(value), status: "Confirmada" }); }; return <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><form className="sale-modal" onSubmit={submit}><div className="modal-head"><div><span className="panel-kicker">NOVA OPERAÇÃO</span><h3>Registrar venda</h3><p>Seu nome, equipe e hierarquia serão preenchidos pelo seu acesso.</p></div><button type="button" onClick={onClose}><X size={19} /></button></div><div className="form-grid single"><label>Valor da venda<input type="number" min="1" step="0.01" value={value} onChange={(e) => setValue(e.target.value)} placeholder="0,00" required autoFocus /></label></div><div className="modal-note"><Zap size={17} /><span><strong>Tempo real ativado.</strong> A venda aparecerá automaticamente na Central TV.</span></div><div className="modal-actions"><button type="button" className="outline-btn" onClick={onClose} disabled={saving}>Cancelar</button><button className="primary-btn" disabled={saving || !value}><Check size={16} /> {saving ? "Confirmando..." : "Confirmar venda"}</button></div></form></div>; }
...
function TvPanel({ sales, announcement, onExit }: { sales: Sale[]; announcement: Sale | null; onExit: () => void }) {
  ...
  return <div className="tv-screen">
    ...
    {featuredSale ? <section className="sale-celebration" aria-live="assertive">
      ...
      <div className="airport-board"><div className="board-title"><span>ÚLTIMAS OPERAÇÕES</span><span><i /> AO VIVO • ATUALIZAÇÃO AUTOMÁTICA</span></div><div className="board-head"><span>HORA</span><span>VENDEDOR</span><span>SUPERVISOR</span><span>EQUIPE</span><span>VALOR</span><span>STATUS</span></div>{sales.length ? sales.slice(0, 7).map((s, i) => <div className={`board-row ${i === 0 ? "highlight" : ""}`} key={s.id}><strong>{s.time}</strong><span className="board-person"><b>{initials(s.seller)}</b>{s.seller}</span><span>{s.supervisor}</span><span>{s.team}</span><strong className="board-money">{money(s.value)}</strong><span className="board-status"><i /> CONFIRMADA</span></div>) : <div className="tv-empty"><CircleDollarSign size={34} /><strong>Aguardando a primeira venda</strong><span>As novas operações aparecerão aqui automaticamente.</span></div>}</div>
